...
- Standing owner authorization: publish every completed, verified update without asking again. Push its commit and matching annotated version tag to the configured `kairo8080/gacha` repository, create the GitHub release, verify the connected Vercel deployment and visible website version, and report that version. If publication fails, report the blocker rather than claiming the update is live.
- Rollback history: retain all prior version tags and GitHub releases. Never overwrite a released tag or force-push to rewrite published history. When the owner requests a rollback, redeploy the requested tagged version or restore that version's code in a new versioned release, as appropriate to the request.
